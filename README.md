...
- Imported the CSV file into **DBeaver** and performed data quality assessment by checking data types, missing values, duplicate records, and descriptive statistics.
- Cleaned and standardized the data to ensure accuracy and consistency for subsequent analysis.

2. **SQL Script:**

The script is written in MySQL. The code is then loaded into Tableau for visualization and dashboard development.

**SQL Techniques Used**

- **`CTEs`** and **`CASE WHEN`** to define funnel stages.
- **`COUNT (DISTINCT)`** to count users.
- Date functions like **`DATE_SUB()`**, **`DATE()`**, and **`MAX()`** to filter the latest 30 days of activity, and **`TIMESTAMPDIFF()`** to calculate the average time users spend between funnel stages.
...
3. **Tableau Dashboard:** [View Dashboard](https://public.tableau.com/views/Book1_17814571393870/Overview?:language=en-US&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link)
